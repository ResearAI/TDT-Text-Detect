...
class DelegateDetector:
    ...
    def _get_feature_fields(self, name2d):
        ...
        # Check if this is a wavelet-based detector
        if name2d and 'wave' in self.detector_name:
            # For wavelet features, we need to handle multiple components
            fields = []
            for ch in name2d:
                base_field = abbr_fields[ch]
                # Add three wavelet components for each field
                fields.extend([f'{base_field}_wave1', f'{base_field}_wave2', f'{base_field}_wave3'])
            return fields
        ...
        for item in tqdm.tqdm(items, desc=f"Computing {self.detector_name} criteria"):
            ...
            for field in self.eval_fields:
                if field in item:
                    ...
                    # Handle vector features from wavelet analysis
                    if isinstance(crit, list) and len(crit) == 3:
                        # Store wavelet features as separate components
                        result[f'{field}_wave1'] = crit[0]
                        result[f'{field}_wave2'] = crit[1]
                        result[f'{field}_wave3'] = crit[2]
                        # Also store as combined for backward compatibility
                        result[f'{field}_crit'] = crit[0]  # Use first component as primary
                    else:
                        # Scalar feature
    ...
    def _fit_threshold(self, datasets, category, label_fn):
        ...
        # Check if we're dealing with wavelet features
        if 'wave' in self.detector_name and len(self.feature_fields) > 1:
            # For wavelet detectors in 1D mode, use the first wavelet component
            field = self.feature_fields[0].replace('_wave1', '')
            pairs = [(item[f'{field}_wave1'], label_fn(item)) for item in results if f'{field}_wave1' in item]
        else:
            field = self.feature_fields[0]
    ...
    def _fit_model(self, datasets, category, label_fn):
        ...
        random.shuffle(results)
        # prepare data
        # Extract features based on field names
        pairs = []
        for item in results:
            label = label_fn(item)
            if label is None:
            ...
            for field in self.feature_fields:
                if field in item:
                    # Direct field access for wavelet components
                    features.append(item[field])
                elif field.replace('_wave1', '_crit').replace('_wave2', '_crit').replace('_wave3', '_crit') in item:
                    # Fallback to _crit field
                    features.append(item[field.replace('_wave1', '_crit').replace('_wave2', '_crit').replace('_wave3', '_crit')])
                else:
                    features.append(0)
            
            pairs.append((features, label))
        
        pairs = [([0 if str(c) == 'nan' else c for c in cs], l) for cs, l in pairs]
        pairs_pos = [(c, l) for c, l in pairs if l]
        pairs_neg = [(c, l) for c, l in pairs if not l]
        if self.args.ndev > 2:
            pairs_pos = pairs_pos[:self.args.ndev // 2]
    ...
    def _predict_threshold(self, config, model, item):
        ...
        assert len(self.feature_fields) == 1 or ('wave' in self.detector_name and len(self.feature_fields) == 3)
        
        if 'wave' in self.detector_name and len(self.feature_fields) == 3:
            # For wavelet detector in 1D mode, use first component
            field = self.feature_fields[0].replace('_wave1', '')
            if f'{field}_wave1' in item:
                crit = item[f'{field}_wave1']
            else:
                crit = item.get(f'{field}_crit', 0)
        else:
            field = self.feature_fields[0]
    ...
    def _predict_model(self, config, model, item):
        ...
        for field in self.feature_fields:
            if field in item:
                ...
                # Fallback to _crit field
                features_list.append(item[field.replace('_wave1', '_crit').replace('_wave2', '_crit').replace('_wave3', '_crit')])
            else:
    ...
    def _eval(self, results, config, model, label_fn):
        ...
        for item in results:
            ...
            if hasattr(self.detector, 'get_dynamic_threshold'):
                field = self.feature_fields[0]
                if 'wave' in self.detector_name and len(self.feature_fields) >= 3:
                    # For wavelet detector, use the base field name
                    field = field.replace('_wave1', '')
                if field in item:
                    target = item[field]
                    # Recompute to get perplexity
                    crit = self.detector.compute_crit(target)
                    if isinstance(crit, list) and len(crit) == 3:
                        # Store wavelet components
                        item[f'{field}_wave1'] = crit[0]
                        item[f'{field}_wave2'] = crit[1]
                        item[f'{field}_wave3'] = crit[2]
                        item[f'{field}_crit'] = crit[0]  # Use first component
                    else:
